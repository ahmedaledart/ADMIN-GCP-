...
export default function Settings() {
  ...
  const fetchSettings = async () => {
    try {
      ...
      // If no row exists, we might need to handle it or create one, but generally it's seeded.
      if (err) throw err;
      setSettings(data);
    } catch (err: any) {
      console.error(err);
  ...
  if (loading && !settings) return <div className="p-8 text-center">جاري التحميل...</div>;
  if (!settings && !loading) return <div className="p-4 bg-red-50 text-red-600 rounded-lg">لم يتم العثور على الإعدادات</div>;
